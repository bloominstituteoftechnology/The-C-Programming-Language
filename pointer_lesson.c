...
int* childOfMainWithoutAllocation() {
  // Here, we don't use `malloc`. We just statically allocate an array to hold 10 integers.
  int integerArray[10];
  for (int i = 0 ; i < 10 ; ++i) {
    integerArray[i] = i;
  }
  // The `&` operator is the inverse of the `*` operator. It outputs the actual memory address 
  // of its operand, not the operand's value.
  return &integerArray;
}
...
// i.e. A node that references both the node that comes before it in the list, as well as the node that comes after it
...
// A function that allocates the necessary memory and initializes the nodes with NULL `next` and `last` fields. 
// It should return an instance of the `radDoublyLinkedList` struct.
struct radDoublyLinkedList* doTheStructAssignmentTest() {
  // Implement this function
  struct radDoublyLinkedList* newList = malloc(sizeof(struct radDoublyLinkedList));
  newList->next = NULL;
  newList->last = NULL;
  return newList;
}
...
// More memory will need to be allocated to the node, and its struct properties need to be initialized accordingly
void radDoublyLinkedListAdd(struct radDoublyLinkedList* list, int value) {
  // Implement this function
  struct radDoublyLinkedList* node = doTheStructAssignmentTest();
  node->value = value;
...
// You'll need to implement the `doTheStructAssignmentTest` function and the `radDoublyLinkedListAdd` function for this code to compile.
